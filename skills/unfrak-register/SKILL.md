...
description: >-
  Third stage of a DDD retrofit: record every place the code does not do what the user says it should, as a durable register of intent gaps with evidence, cost, and the test that will prove each one closed. Use whenever `unfrak-ratify` returns a "wrong" verdict, and when closing an entry after a later fix.
---
...
It lives in one document (`INTENT_GAPS.md`, or whatever the project already uses), and it is a queue: entries are added by ratification and removed by fixes.

## What earns an entry
Any behavior the user said is wrong — whether it looks like a bug, an oversimplification, or a decision that has been outgrown. The distinction does not matter here. What matters is that the documentation now promises something the code does not deliver.
...
- **The documentation line it belongs to**, so the entry and the promise stay connected.
